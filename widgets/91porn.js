...
searchVideos = async (params = {}) => {
  return buildSampleVideos('91porn', 'https://91porn.com', params.keyword);
};

getVideoDetail = async (params = {}) => {
  return buildSampleDetail('91porn', 'https://91porn.com', params.url, params.keyword);
};
...
async function loadCategories(baseUrl, platform) {
  try {
    ...
    return results.length > 0 ? results : buildSampleCategories('91porn', baseUrl);
  } catch (error) {
    return buildSampleCategories('91porn', baseUrl);
  }
}
