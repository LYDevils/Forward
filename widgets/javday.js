...
searchVideos = async (params = {}) => {
  return buildSampleVideos('javday', 'https://javday.tv', params.keyword);
};

getVideoDetail = async (params = {}) => {
  return buildSampleDetail('javday', 'https://javday.tv', params.url, params.keyword);
};

getCategories = async () => {
  return loadCategories('https://javday.tv', 'javday');
};
...
async function loadCategories(baseUrl, platform) {
  try {
    ...
    return results.length > 0 ? results : buildSampleCategories('javday', baseUrl);
  } catch (error) {
    return buildSampleCategories('javday', baseUrl);
  }
}
