...
async function extractVideo(url) {
  try {
    ...
    throw new Error(`Failed to extract video from jable: ${error.message}`);
  }
}
...
async function search(keyword, options = {}) {
  try {
    ...
    const searchUrl = `${BASE_URL}/search/${encodeURIComponent(keyword)}/page/${page}`;
    ...
    throw new Error(`Failed to search jable: ${error.message}`);
  }
}
...
async function getVideoInfo(url) {
  try {
    ...
    throw new Error(`Failed to get video info from jable: ${error.message}`);
  }
}
