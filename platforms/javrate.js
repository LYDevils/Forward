...
async function extractVideo(url) {
  try {
    ...
    throw new Error(`Failed to extract video from javrate: ${error.message}`);
  }
}
...
async function search(keyword, options = {}) {
  try {
    ...
    throw new Error(`Failed to search javrate: ${error.message}`);
  }
}
...
async function getVideoInfo(url) {
  try {
    ...
    throw new Error(`Failed to get video info from javrate: ${error.message}`);
  }
}
